...
void solve() {
    ...
    set<array<int,2>> vis;
    stack<array<int,3>> nmb;
    int xx,yy,zz,cnt;bool gh=0;
    auto dfs=[&](auto self,int x,int y,int fg)->void
    {
        ...
        if(fg==0)
        {
            ...
            {
                if(fg!=inv(fg,s[x-1][y])&&!vis.count({x-1,y})) vis.insert({x-1,y}),nmb.push({x,y,fg});
        ...
        else if(fg==2)
        {
            ...
            {
                if(fg!=inv(fg,s[x][y-1])&&!vis.count({x,y-1})) vis.insert({x,y-1}),nmb.push({x,y,fg});
                road.push_back({x,y,fg});
